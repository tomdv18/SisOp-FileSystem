...
struct Directory {
	...
	time_t last_access;
	time_t last_modification;
	mode_t permissions;  // Campo para almacenar permisos
	uid_t owner;
	gid_t group;
};
...
struct Master {
	...
	struct Symlink *symlinks[MAX_LINKS_DIREC];  // Assuming a directory can have up to 5 symlinks
	int numSymlinks;
	struct File **files;
	int numFiles;
};
...
{
	printf("[debug] AAAAAAAAAAAAAAAA fisopfs_chmod - %s\n", path);
...
static int
fisopfs_chown(const char *path, uid_t owner)
{
	printf("[debug] fisopfs_chown - %s\n", path);
...
int
has_read_permission(const struct File *file)
{
	uid_t user_id = getuid();

	// Verifico si el usuario es propietario del archivo o tiene privilegios de root
	if (user_id == file->owner || user_id == 0) {
		return 1;
	}

	// Verifico si el grupo del usuario coincide con el grupo del archivp
	gid_t user_group = getgid();
	if (user_group == file->group) {
		if ((file->permissions & S_IRGRP) != 0) {
			return 1;  //
		}
	}
	// Otros users
	if ((file->permissions & S_IROTH) != 0) {
		return 1;
	}

	// Si no se cumple ninguna de las condiciones anteriores, el usuario no tiene permisos de lectura
	return 0;
}


int
has_write_permission(const struct File *file)
{
	...
	gid_t user_group = getgid();
	if (user_group == file->group) {
		if ((file->permissions & S_IWGRP) != 0) {
			return 1;
		}
	}

	// Otros users
	if ((file->permissions & S_IWOTH) != 0) {
		return 1;
	}
...
/* FIN desafio PERMISOS */

/* Desafio symlink */
int
rootHasSymlink(const char *path)
{
	printf("[debug] rootHasSymlink - %s\n", path);

	if (!root)
		printf("AAAAAAAAAAAAAAAAAA");
	for (int i = 0; i < MAX_LINKS_DIREC; i++) {
...
struct Symlink *
createSymlink(const char *name, const char *target)
{
	printf("[debug] createSymlink: Name: %s, target: %s\n", name, target);
	struct Symlink *newLink =
	        (struct Symlink *) malloc(sizeof(struct Symlink));
	strncpy(newLink->name, name, sizeof(newLink->name) - 1);  // Copy the name
	newLink->name[sizeof(newLink->name) - 1] = '\0';
	strncpy(newLink->target,
	        target,
	        sizeof(newLink->target) - 1);  // Copy the target
...
int
addSymlink(const char *name, const char *target)
{
...
void
saveSymlink(FILE *archivo, struct Symlink *sLink)
{
...
static int
fysopfs_readlink(const char *path, char *buf, size_t size)
{
...
createFile(const char *name)
{
	...
	newFile->permissions = FILE_ST_MODE;
	newFile->owner = geteuid();
...
fisopfs_destroy(void *private_data)
{
	...
	for (int k = 0; k < MAX_LINKS_DIREC; k++) {
		if (root->symlinks[k])
...
int
addDirectoryToRoot(const char *path)
{
	printf("[debug] fisopfs_mkdir: addDirectoryToRoot - path: %s\n", path);

	// if (!userHasPermission(path, W_OK | X_OK)) {
	//  return -EACCES;  // Permiso denegado
	// }

	for (int i = 0; i < MAX_DIRECS_DIREC; i++) {
		if (!root->subdirectories[i]) {
			root->subdirectories[i] = (struct Directory *) malloc(
			        sizeof(struct Directory));
			// memset(root->subdirectories[i]->files, 0, sizeof(root->subdirectories[i]->files));
			strncpy(root->subdirectories[i]->name,
			        path,
			        sizeof(root->subdirectories[i]->name) - 1);
			root->subdirectories[i]
			        ->name[sizeof(root->subdirectories[i]->name) - 1] =
			...
			root->subdirectories[i]->last_modification =
			        times[1].tv_sec;
			root->subdirectories[i]->permissions = FOLDER_ST_MODE;
			root->subdirectories[i]->owner = geteuid();
...
fisopfs_mkdir(const char *path, mode_t mode)
{
	...
	path_plus_one++;
	if (root->numSubdirectories < MAX_DIRECS_DIREC &&
	    strchr(path_plus_one, '/') == NULL) {
	...
	if (strlen(folder) > 0 && strlen(file) == 0) {
		...
		/*for (int i = 0; i < root->numFiles; i++) {
		        if (!root->files[i]) {
		                ...
		                root->numFiles++;
		                printf("[debug] fisopfs_create - files in root:
		%d\n", root->numFiles); return 0;
		        }
		}*/
		...
		for (int i = 0; i < MAX_FILES_DIREC; i++) {
		        if (!dir->files[i]) {
		                ...
		                dir->numFiles++;
		                printf("[debug] fisopfs_create - files in
		folder: %d\n", dir->numFiles); return 0;
		        }
		}*/
...
delFile(struct File *f, int *num_files, struct File ***arr)
{
	...
	char sname[strlen(f->name) + 2];  // +2 para '/' y '\0'
	sname[0] = '/';
	strcpy(sname + 1, f->name);
	struct Symlink *s = searchSymlink(sname);
	if (s) {
		delSymlink(s);
	}
...
static int
my_getattr(const char *path, struct stat *st)
{
	return foo("getattr");
}
...
static int
my_mknod(const char *path, mode_t mode, dev_t dev)
{
	return foo("mknod");
}
...
static int
my_symlink(const char *target, const char *linkpath)
{
	return foo("symlink");
}
...
static int
my_link(const char *oldpath, const char *newpath)
{
	return foo("link");
}
...
static int
my_flush(const char *path, struct fuse_file_info *fi)
{
	return foo("flush");
}
